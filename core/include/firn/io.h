...
std::optional<Image16> load16_memory(const uint8_t* data, size_t size);   // nullopt for 8-bit data
bool save_png16(const Image16& img, const std::string& path, std::string* err = nullptr);
std::vector<uint8_t> encode_png16(const Image16& img);   // empty on failure
